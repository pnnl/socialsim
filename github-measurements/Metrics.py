...
def js_divergence(ground_truth, simulation, base=np.e):
...
def rmse(ground_truth,simulation,join='inner',fill_value=0):
    ...
    Inputs:
    ground_truth - ground truth measurement (data frame)
    simulation - simulation measurement (data frame)
    join - type of join to perform between ground truth and simulation
    fill_value - fill value for non-overlapping joins
    """

    grouped = pd.concat([ground_truth, simulation], join=join, axis=1).fillna(fill_value)
    grouped.columns = ["ground_truth","simulation"]
    return np.sqrt(((grouped["simulation"] - grouped["ground_truth"]) ** 2).mean())
...
def r2(ground_truth,simulation,join='inner',fill_value=0):
    ...
    grouped = pd.concat([ground_truth, simulation], join=join, axis=1).fillna(fill_value)
    grouped.columns = ["ground_truth", "simulation"]
    return r2_score(grouped["ground_truth"],grouped["simulation"])
...
def pearson(ground_truth,simulation,join='inner',fill_value=0):
    ...
    grouped = pd.concat([ground_truth, simulation], join=join, axis=1).fillna(fill_value)
    grouped.columns = ["ground_truth", "simulation"]
    return pearsonr(grouped["ground_truth"],grouped["simulation"])
...
def main():
    ...
    print('KS test')
    gt,sim,metric = get_metric_scores(ground_truth,simulation,getUserPopularity,ks_test,measurement_kwargs={'k':1000})
    ...
    print('JS divergence')
    gt,sim,metric = get_metric_scores(ground_truth,simulation,getUserPopularity,js_divergence,measurement_kwargs={'k':1000})
    print('User Popularity',metric)
    ...
    distxy = fast_dtw(x,y); distxz = fast_dtw(x,z); distyz = fast_dtw(y,z)
    print "dist(x,y) = ", distxy, " dist(x,z) = ", distxz, " dist(y,z) = ", distyz

    print "Using DTW"
    distxy = dtw(x,y); distxz = dtw(x,z); distyz = dtw(y,z)
    print "dist(x,y) = ", distxy, " dist(x,z) = ", distxz, " dist(y,z) = ", distyz
