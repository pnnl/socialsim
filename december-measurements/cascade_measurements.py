...
class Cascade:
    ...
    def get_cascade_depth(self):
        try:
            return max([nx.shortest_path_length(self.cascade_nx, x, self.root_node) for x in self.cascade_nx.nodes() if
                        self.cascade_nx.in_degree(x) == 0])
        except:
            return None
    ...
    def get_cascade_breadth(self):
        return max([self.cascade_nx.in_degree(x) for x in self.cascade_nx.nodes()])
...
class SingleCascadeMeasurements:
    ...
    def get_depth_based_measurements(self):
        ...
        while len(seed_nodes) > 0:
            ...
            seed_nodes = self.main_df[self.main_df[self.parent_node_col].isin(seed_nodes)][self.node_col].values
            assert len(set(seed_nodes)) == len(seed_nodes)
            depth += 1
        print(self.main_df)
        depth_based_measurements = []
        old_unique_nodes_count = 1  # root node, since we start iterating from depth 1
        # print("this", list(range(1, max(self.main_df['depth'])+1)))
        ...
        # print("depth_based_measurements", depth_based_measurements)
    ...
    def cascade_participation_palma(self):
        print(self.node_participation())
        return palma_ratio(self.node_participation())

    def node_participation(self):
        print('main_df',self.main_df.columns)
        return self.main_df.groupby(self.main_df["true_{}".format(self.node_col)]).size().reset_index(name='counts')['counts'].values
...
class CascadeCollectionMeasurements:
    ...
    def __init__(self, main_df, parent_node_col="parentID", node_col="nodeID", root_node_col="rootID", timestamp_col="nodeTime",user_col = "nodeUserID",
        ...
        try:
            df['communityID'] = df['attributes'].apply(lambda x: eval(x)['communityID'])
        except:
            ''   
    ...
    def get_cascade_collection_timeline_timeseries(self, time_granularity="M", community_grouper=None):
        """
         ...
         """
        temporal_measurements = []
        result_df_columns = ["timestamp", "lifetime"]
        ...
        if community_grouper and community_grouper in self.main_df.columns:
            ...
            result_df_columns = ["timestamp", community_grouper, "lifetime"]
        for ts, df in self.main_df[self.main_df[self.node_col] == self.main_df[self.root_node_col]]. \
            ...
            temporal_measurements.append(ts + [mean_lifetime] if community_grouper and community_grouper in self.main_df.columns else [ts, mean_lifetime])
    ...
    def get_cascade_collection_size_timeseries(self, time_granularity="M", community_grouper=None):
        ...
        """
        temporal_measurements = []
        result_df_columns = ["timestamp", "size"]
        grouper = [pd.Grouper(freq=time_granularity)]
        if community_grouper and community_grouper in self.main_df.columns:
            grouper.append(community_grouper)
            result_df_columns = ["timestamp", community_grouper, "size"]
        for ts, df in self.main_df[self.main_df[self.node_col] == self.main_df[self.root_node_col]]. \
                set_index(self.timestamp_col).groupby(grouper, sort=True):
            ...
            temporal_measurements.append(ts + [mean_size] if community_grouper and community_grouper in self.main_df.columns else [ts, mean_size])
            #temporal_measurements.append([*ts, mean_size] if community_grouper else [ts, mean_size])
        meas = pd.DataFrame(temporal_measurements, columns=["time",community_grouper,"value"])
        if len(meas.index) == 0:
            return None
        else:
            return meas
    ...
    def get_community_users_count_timeseries(self, time_granularity="M", community_grouper=None):
        ...
        if community_grouper in self.main_df.columns:
            for community_identifier, community_df in self.main_df.groupby(community_grouper):
                ...
                self.community_users_count_timeseries_df = pd.DataFrame(temporal_measurements, columns=["timestamp", community_grouper, "unique_users", "new_user_ratio"])
    ...
    def community_users_count(self, attribute, time_granularity, community_grouper):
        ...
        if community_grouper in self.main_df.columns:
            if time_granularity not in self.community_users_count_timeseries_df:
                self.get_community_users_count_timeseries(time_granularity)
            return self.community_users_count_timeseries_df[time_granularity][["timestamp", community_grouper, attribute]]
        else:
            return None
