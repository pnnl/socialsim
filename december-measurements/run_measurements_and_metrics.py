...
def run_metrics(ground_truth, simulation, measurement_name, measurement_params,
    ...
    if not isinstance(ground_truth, dict):
        measurement_on_gt = run_measurement(ground_truth, measurement_name, measurement_params, plot_flag=False,
                                            simulation=False)
    else:
        measurement_on_gt = ground_truth[measurement_name]

    measurement_on_sim = run_measurement(simulation, measurement_name, measurement_params, plot_flag=False,
                                         simulation=True)
...
def generate_plot(simulation=None, ground_truth=None, measurement_name='', show=False, plot_dir=''):
    ...
    if measurement_name in measurement_plot_params and not (simulation is None and ground_truth is None) and \
            not ((not simulation is None and len(simulation) == 0) and (
                    not ground_truth is None and len(ground_truth) == 0)):
...
def run_all_metrics(ground_truth, simulation, measurement_params, filters={}, plot_flag=False,
    ...
    for measurement_name in measurements:
        gt, sim, metric_results = run_metrics(ground_truth, simulation, measurement_name, measurement_params,
                                              plot_flag=plot_flag,
                                              show=show, plot_dir=plot_dir)
...
def cascade_examples(platform):
    ...
    # run individual metric
    gt_measurement, sim_measurement, metric = run_metrics(gt_measurements, sim_measurements,
